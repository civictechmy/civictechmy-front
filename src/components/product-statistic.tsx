...
export default function ProductStatistic({
  ...
  const completed =
    products.filter(
      (product) =>
        product.status?.find(
          (status) => status.isActive && status.phase_index === "1",
        ) || false,
    ).length || 0;
  ...
  const under_dev =
    products.filter(
      (product) =>
        product.status?.find(
          (status) => status.isActive && status.phase_index === "0",
        ) || false,
    ).length || 0;
  ...
  return (
    <div className="flex w-full flex-wrap items-start gap-6 md:flex-nowrap">
      <div className="flex-1 space-y-3">
        <p className="text-txt-govtech-600 text-heading-lg font-semibold">
          {total}
        </p>
